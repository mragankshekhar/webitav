...
if ($pgAct == "viewall") {
    ...
    if (isset($_REQUEST["q"]) && $_REQUEST["q"] != "") {
        ...
        array_push($sqlArray, $_REQUEST["page_id"]);
        $whereAry[] = " (page_type = ?) ";
    }
    ...
    $pcount = $db->getVal("select count(id) from  " . SETTINGS . " $where", $sqlArray);
	//echo "error==><pre>".$db->getlq();exit;
	
    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,field,en,input_type,page_type from " . SETTINGS . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    if (is_array($contentDetail) && count($contentDetail) > 0) {
        ...
        foreach ($contentDetail as $iList) {
            ...
            $aryPgAct["page_id"] = $pgMod;
			 ...
			 if($iList['en']=='text')
			 $value=$iList['en'];
			 elseif($iList['en']=='textarea')
			 $value=substr($iList['en'],0,100);
			 elseif($iList['en']=='image')
			 $value="<img src='".URL_ROOT."uploads/content/".$iList['en']."' width='50'  />";
            ...
            $aryData[] = array(
                $checkbox,
                "<a href='" . URL_ADMIN_HOME . getQueryString($aryPgAct) . "'>" . $iList["field"] ."</a>",
	...
	elseif ($pgAct == "edit" && isset($_POST['id']) && trim($_POST['id']) != '') {
       ...
       if (isset($_POST["status"]))
		...
		$sqlArray = array($_POST['id']);
            ...
            $flgUp = $db->updateAry(SETTINGS, $aryData, "where id=?", $sqlArray);
            if (!is_null($flgUp)) {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
                echo "success==>" . (URL_ADMIN_HOME . getQueryString(array("page_id" => $pgMod)));
    ...
    $res = $db->delete("delete from " . SETTINGS . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . SETTINGS . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(SETTINGS, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(SETTINGS, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(SETTINGS, array("status" => 1), "where id='" . $ids . "'");
        }
    }
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . SETTINGS . " WHERE id=?", $sqlUserEditArray);
                        }
                ...
                <div class="form-group">
                  ...
                  <div class="col-lg-10">
                    <input class="submit btn btn-blue" type="submit" value="Submit" />
						...
						if (isset($_REQUEST["page_id"]) && $_REQUEST["page_id"] != "") {
                            $extraAry = "&page_id=".$_REQUEST["page_id"];
                        }
                        if ($extraAry!="")
                        ...
                        ?>" data-page="<?php echo $pgMod ?>" data-table="<?php echo $pgTable ?>" class="table table-widget table-striped" id="mssresulttable" data-export="0,4,5">
                            <thead>
                                <tr>
                                    ...
                                    <th class="first"><a href="#" title="linkname">Field Name</a></th>
                                   <th class="first"><a href="#" title="linkname">Value</a></th>
                                    <th><a href="#" title="linkname">Page Type</a></th>
