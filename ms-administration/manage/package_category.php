...
$pgMod = "package_category";
$pgAct = "view";
$pgTable = MCAT;
$pgHeading = "Package Category";
...
if ($pgAct == "viewall") {
    ...
    $pcount = $db->getVal("select count(id) from  " . MCAT . " $where", $sqlArray);

    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,catname,detail,status from " . MCAT . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    if (is_array($contentDetail) && count($contentDetail) > 0) {
        ...
        foreach ($contentDetail as $iList) {
            ...
            $aryData[] = array(
                $checkbox,
                "<a href='" . URL_ADMIN_HOME . getQueryString($aryPgAct) . "'>" . $iList["catname"] ."</a>",
    ...
    if ($pgAct == "add") {
       ...
       if (isset($_POST["status"]))
		...
		{$flgEr=TRUE;
			...
			$flgIn=$db->insertAry(MCAT,$aryData);
			if(!is_null($flgIn))
			{
	...
	elseif ($pgAct == "edit" && isset($_POST['id']) && trim($_POST['id']) != '') {
       ...
       if (isset($_POST["status"]))
		...
		$flgUp=$db->updateAry(MCAT,$aryData," where id=".$_REQUEST["id"]);
    ...
    $details = $db->getRow("select username,email,avatar from " . MCAT . " where id=?", $sqlArray);
    $res = $db->delete("delete from " . MCAT . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . MCAT . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(MCAT, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(MCAT, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(MCAT, array("status" => 1), "where id='" . $ids . "'");
        }
    }
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . MCAT . " WHERE id=?", $sqlUserEditArray);
                        }
    ...
    <div class="form-group">
      ...
      <div class="col-lg-10">
        ...
        <input type="text" name="name" id="name" value="<?php echo unPOST($aryForm['catname']); ?>" class="form-control" />
    ...
    <div class="form-group">
      <label class="col-lg-2 control-label" for="body"> Detail</label>
      <div class="col-lg-10">
        <textarea name="detail" class="form-control textarea" rows="10" ><?php echo unPOST($aryForm['detail']); ?></textarea>
      </div>
    </div>
    ...
    <div class="form-group">
                  <label class="col-lg-2 control-label" for="linkname"> Status</label>
                  <div class="col-lg-10">
