...
if ($pgAct == "viewall") {
    ...
    $pcount = $db->getVal("select count(id) from  " . CONTACT . " $where", $sqlArray);

    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,name,email,mobile,comments,udate,status,language from " . CONTACT . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    if (is_array($contentDetail) && count($contentDetail) > 0) {
        ...
        foreach ($contentDetail as $iList) {
            ...
            if ($_SESSION["admin"]["adminroll"] == 1 || ($permission == "a" || $permission == "w")) {
                ...
                $button .= "<li><a onclick='loc(\"" . URL_ADMIN_HOME . getQueryString($aryPgAct) . "\")'><i class='fa fa-edit'></i> View</a></li>";
            }
    ...
    if ($pgAct == "add") {
       ...
       if (isset($_POST["status"]))
		...
		$flgIn = $db->insertAry(CONTACT, $aryData);
	...
	elseif ($pgAct == "edit" && isset($_POST['id']) && trim($_POST['id']) != '') {
       ...
       if (isset($_POST["status"]))
		...
		$sqlArray = array($_POST['id']);
            ...
            $flgUp = $db->updateAry(CONTACT, $aryData, "where id=?", $sqlArray);
            if (!is_null($flgUp)) {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
                echo "success==>" . (URL_ADMIN_HOME . getQueryString(array("page_id" => $pgMod)));
    ...
    $details = $db->getRow("select username,email,avatar from " . CONTACT . " where id=?", $sqlArray);
    $res = $db->delete("delete from " . CONTACT . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . CONTACT . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(CONTACT, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(CONTACT, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(CONTACT, array("status" => 1), "where id='" . $ids . "'");
        }
    }
        ...
        <div class="col-md-12">
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . CONTACT . " WHERE id=?", $sqlUserEditArray);
                        }
              ...
              <div class="form-group">
                  <label class="col-lg-2 control-label" for="type"> Name</label>
                  <div class="col-lg-10">
                    	<?php echo unPOST($aryForm['name']); ?>
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-lg-2 control-label" for="type"> Date</label>
                  <div class="col-lg-10">
                    	<?php echo unPOST($aryForm['udate']); ?>
                  </div>
                </div>
              <div class="form-group">
                  <label class="col-lg-2 control-label" for="Name"> Email </label>
                  <div class="col-lg-10">
                    <?php echo unPOST($aryForm['email']); ?>
                  </div>
                </div>	
				<div class="form-group">
                  <label class="col-lg-2 control-label" for="linkname"> Mobile </label>
                  <div class="col-lg-10">
                    <?php echo unPOST($aryForm['mobile']); ?>
                  </div>
                </div>
				<div class="form-group">
                  <label class="col-lg-2 control-label" for="pbody"> Content </label>
                  <div class="col-lg-10">
                  	<?php echo unPOST($aryForm['comments']); ?>
                  </div>
                </div>	          
                        ...
                        ?>" data-page="<?php echo $pgMod ?>" data-table="<?php echo $pgTable ?>" class="table table-widget table-striped" id="mssresulttable" data-export="0,4,5">
                            <thead>
                                <tr>
                                    ...
                                    <th class="first"><a href="#" title="linkname">Date</a></th>
                                    <th><a href="#" title="linkname">Email</a></th>
                                    <th><a href="#" title="linkname">Mobile</a></th>
