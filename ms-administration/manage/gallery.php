...
if ($pgAct == "viewall") {
    ...
    $pcount = $db->getVal("select count(id) from  " . GALLERY . " $where", $sqlArray);

    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,name,image,status,language from " . GALLERY . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    if (is_array($contentDetail) && count($contentDetail) > 0) {
        ...
        foreach ($contentDetail as $iList) {
            ...
            $image="<img src='".URL_ROOT."uploads/avatar/".$iList['image']."' width='50'  />";
    ...
    if ($pgAct == "add") {
       ...
       if (isset($_POST["status"]))
		...
		$flgIn = $db->insertAry(GALLERY, $aryData);
	...
	elseif ($pgAct == "edit" && isset($_POST['id']) && trim($_POST['id']) != '') {
       ...
       if (isset($_POST["status"]))
		...
		$sqlArray = array($_POST['id']);
            ...
            $flgUp = $db->updateAry(GALLERY, $aryData, "where id=?", $sqlArray);
            if (!is_null($flgUp)) {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
                echo "success==>" . (URL_ADMIN_HOME . getQueryString(array("page_id" => $pgMod)));
    ...
    $details = $db->getRow("select username,email,avatar from " . GALLERY . " where id=?", $sqlArray);
    $res = $db->delete("delete from " . GALLERY . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . GALLERY . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(GALLERY, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(GALLERY, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(GALLERY, array("status" => 1), "where id='" . $ids . "'");
        }
    }
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . GALLERY . " WHERE id=?", $sqlUserEditArray);
                        }
                  ...
                  <div class="col-lg-10">
					...
					<?php
					  if($_GET['action']=="edit" && $aryForm['image']!='' && $ext[1]!="")
					  {
						 ...
						 ?>
						  <br/>
						  <img src="../uploads/avatar/<?php echo $aryForm['image'];?>" width="100" />
