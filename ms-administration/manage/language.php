...
if ($pgAct == "viewall") {
    ...
    $pcount = $db->getVal("select count(id) from  " . LANGUAGE . " $where", $sqlArray);

    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,name,code,status from " . LANGUAGE . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    if ($pgAct == "add") {
       ...
       if (isset($_POST["status"]))
         ...
         if ($flgEr != TRUE) {
           $aryData=array(	'code'	=>	$_POST['code'],
						'name'	=>	POST('name'),
						'status'	=>$status);
		
		
		$flgIn = $db->insertAry(LANGUAGE, $aryData);
	...
	elseif ($pgAct == "edit" && isset($_POST['id']) && trim($_POST['id']) != '') {
       ...
       if (isset($_POST["status"]))
		...
		$sqlArray = array($_POST['id']);
            ...
            $flgUp = $db->updateAry(LANGUAGE, $aryData, "where id=?", $sqlArray);
            if (!is_null($flgUp)) {
                $_SESSION['msg'] = 'Saved Successfully';
                unset($_SESSION['form']);
                echo "success==>" . (URL_ADMIN_HOME . getQueryString(array("page_id" => $pgMod)));
    ...
    $details = $db->getRow("select username,email,avatar from " . LANGUAGE . " where id=?", $sqlArray);
    $res = $db->delete("delete from " . LANGUAGE . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . LANGUAGE . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(LANGUAGE, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(LANGUAGE, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(LANGUAGE, array("status" => 1), "where id='" . $ids . "'");
        }
    }
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . LANGUAGE . " WHERE id=?", $sqlUserEditArray);
                        }
              ...
              <input type="hidden" name="action" value="<?php echo $_GET["action"] ?>" />
                ...
                <div class="form-group">
                  ...
                  <div class="col-lg-10">
                    <input type="text" name="code" id="code" value="<?php echo unPOST($aryForm['code']); ?>" class="form-control" />
                  </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <label class="col-lg-2 control-label" for="linkname">  Is Activate Language ? </label>
                  <div class="col-lg-10">
                        ...
                        ?>" data-page="<?php echo $pgMod ?>" data-table="<?php echo $pgTable ?>" class="table table-widget table-striped" id="mssresulttable" data-export="0,4,5">
                            <thead>
                                <tr>
                                    ...
                                    <th><a href="#" title="linkname">Code</a></th>
