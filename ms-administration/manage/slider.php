...
if ($pgAct == "viewall") {
    ...
    $pcount = $db->getVal("select count(id) from  " . SLIDER . " $where", $sqlArray);

    $startV = $_REQUEST['startV'];
    $endV = $_REQUEST['endV'];
    $ProDetail["totPost"] = $pcount;

    $contentDetail = $db->getRows("select id,heading,detail,link,image,status,language from " . SLIDER . " $where order by id DESC " . ($endV == 'All' ? "" : "limit $startV, $endV"), $sqlArray);
    ...
    $details = $db->getRow("select username,email,avatar from " . SLIDER . " where id=?", $sqlArray);
    $res = $db->delete("delete from " . SLIDER . " where id=?", $sqlArray);
    if (!is_null($res)) {
    ...
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            ...
            $details = $db->getRow("select username,email,avatar from " . SLIDER . " where id=?", $sqlArray);
            @unlink("../uploads/avatar/" . $details["avatar"]);
            $res = $db->delete(SLIDER, "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkinactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(SLIDER, array("status" => 0), "where id='" . $ids . "'");
        }
    }
} elseif ($pgAct == "checkactive") {
    if (is_array($_POST["ids"]) && count($_POST["ids"]) > 0) {
        foreach ($_POST["ids"] as $ids) {
            $res = $db->updateAry(SLIDER, array("status" => 1), "where id='" . $ids . "'");
        }
    }
                ...
                <div class="panel-body">
                    ...
                    if ($pgAct == "add" || ($pgAct == "edit" && isset($_GET['id']) && trim($_GET['id']) != '')) {
                        ...
                        if ($pgAct == "edit" && !isset($_SESSION['form'])) {
                            ...
                            $aryForm = $db->getRow("SELECT * FROM " . SLIDER . " WHERE id=?", $sqlUserEditArray);
                        }
