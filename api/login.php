...
if ($ch == "login") {
    ...
    if (trim($_POST["uid"]) == "") {
        ...
        if (is_array($ids) && count($ids) > 0) {
            ...
            $db->updateAry(SITE_USER, $loginData, " where id=?", array($_SESSION["user"]["uid"]));
            $db->updateAry(PUSH_NOTIFICATION, array("regId" => $_SESSION['reg_id']), " where uid=?", array($ids));
    ...
    if (trim($_POST["fullname"]) == "") {
        ...
        if ($ids > 0 && is_numeric($ids)) {
            $db->insertAry(PUSH_NOTIFICATION, array("uid" => $ids, "regId" => $_SESSION['reg_id']));
            $status["status"] = "success";
            $status["id"] = $ids;
            $_SESSION["user"]["uid"] = $ids["id"];
